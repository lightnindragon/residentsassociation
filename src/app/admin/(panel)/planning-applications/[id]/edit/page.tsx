...
export default async function AdminEditPlanningApplicationPage({
  params,
}: {
  ...
  try {
    ...
    `;
    application = (rows[0] as Row) ?? null;
  } catch {
    // no DB
  }
  ...
  return (
    <div>
      ...
      <h1 className="mt-4 font-heading text-2xl font-semibold">Edit Planning Application</h1>
      {application.archived_at && (
        <p className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950 dark:border-amber-900/40 dark:bg-amber-950/30 dark:text-amber-100">
          This item is <strong>archived</strong> and hidden from the public planning pages. Unarchive
          it from the{" "}
          <Link
            href="/admin/planning-applications?filter=archived"
      ...
      <PlanningApplicationForm authorId={user.id} application={application} />
    </div>
  );
}
