...
import { DeletePlanningApplicationButton } from "./DeletePlanningApplicationButton";
import { ArchivePlanningApplicationButton } from "./ArchivePlanningApplicationButton";
import { formatUkDate } from "@/lib/date-format";

type Filter = "active" | "archived" | "all";

export default async function AdminPlanningApplicationsPage({
  searchParams,
}: {
  ...
  const tab = (name: string, value: Filter) => {
    ...
    return (
      <Link
        href={value === "active" ? "/admin/planning-applications" : `/admin/planning-applications?filter=${value}`}
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Link
          href="/admin/planning-applications/new"
            ...
            {rows.length === 0 ? (
              <tr>
                <td colSpan={4} className="py-8 text-center text-[var(--color-muted)]">
                  {filter === "archived" ? "No archived applications." : "No applications yet."}
                </td>
              </tr>
            ) : (
              rows.map((p) => {
                ...
                return (
                  ...
                  >
                    <td className="py-3">
                      <Link
                        href={`/admin/planning-applications/${p.id}/edit`}
                    ...
                    <td className="py-3 text-right whitespace-nowrap">
                      ...
                      <span className="mr-2 inline-block">
                        <ArchivePlanningApplicationButton applicationId={p.id} archived={archived} />
                      </span>
                      <DeletePlanningApplicationButton applicationId={p.id} />
                    </td>
                  </tr>
                );
              })
