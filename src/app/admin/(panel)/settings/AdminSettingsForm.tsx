...
import { testConnection, sendTestEmail } from "@/app/admin/actions/email-test";
...
export function AdminSettingsForm({ initial }: { initial: Initial }) {
  ...
  const [isConnecting, startConnection] = useTransition();
  ...
  return (
    <>
      ...
      {connectionResult && (
        ...
        >
          {connectionResult.ok ? "Connection successful." : connectionResult.error}
        </p>
      )}
      {testResult && (
        <p
