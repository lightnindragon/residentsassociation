...
export default async function EventDetailPage({
  ...
  try {
    ...
    const rows = await sql`
      SELECT e.id, e.title, e.slug, e.body, e.external_url, e.cover_image_url, e.published_at, e.created_at,
        u.name AS author_name
      FROM site_events e
      LEFT JOIN users u ON u.id = e.author_id
      WHERE e.slug = ${slug} AND e.published_at IS NOT NULL AND e.published_at <= NOW()
        AND e.archived_at IS NULL
      LIMIT 1
    `;
    row = (rows[0] as Row) ?? null;
  } catch (err) {
    console.error("EventDetailPage DB fetch:", err);
  }
  if (!row) notFound();
  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
      <Link href="/events" className="text-sm text-[var(--color-primary)] hover:underline">
        ← Events
      </Link>
