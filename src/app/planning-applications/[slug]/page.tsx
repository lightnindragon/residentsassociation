...
export default async function PlanningApplicationDetailPage({
  params,
}: {
  ...
  try {
    ...
    const rows = await sql`
      SELECT p.id, p.title, p.slug, p.body, p.external_url, p.cover_image_url, p.published_at, p.created_at,
    ...
    console.error("PlanningApplicationDetailPage DB fetch:", err);
  }
  if (!row) notFound();
  ...
  return (
    <article className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
      ...
      <div className="mt-6">
        ...
        >
          Open official planning portal
        </a>
        <p className="mt-2 text-xs text-[var(--color-muted)]">
          You will leave this site to view the application on the planning authority website.
        </p>
      </div>
